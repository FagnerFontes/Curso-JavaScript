...
//As funções em javaScript podem receber parâmetros (valores de entrada) e retornas um valor de saída. Elas podem ser definidas usando a palavra-chave "function", seguido pelo nome da função e os parâmetros entre parêntes. O corpo da função é delimitado por chaves "{}" e contém as instruções que serão executdas quando a função for chamada. Assim: 
...
/* function map(f, a) {
    ...
    var i; //Declaração da variável
    for (i = 0; i != a.length; i++){ ;//Percorre o Array a e aplica a função f a cada elemento, armazenando o resultado no novo Array
    return result = result[i] = f(a[i]);//Retorna o novo Array
    }
    return result; 
  }
